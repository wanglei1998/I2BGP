...
class DebugData:
    ...
    def __init__(self, debug_filename):
        fd = subprocess.check_output (['readelf', "-wi", debug_filename ], universal_newlines=True)
        self.__lines = fd.splitlines ()
        # print(self.__lines)
        self.__current = 0
    ...
    def write_back_one (self, item):
        self.__current = self.__current - 1 - len (list(item.attributes.keys ()))
        return
    def read_one (self):
        item = DebugData.Item ()
        while 1:
...
def search_debug_file():
    ...
    files_to_try.append(os.getenv("LD_LINUX_PATH", "/fake/path"))
...
def main(argv):
    ...
    if debug_filename is None:
        ...
        except CouldNotFindFile:
            print("Library or debug filename not found (perhaps wrong version?), exiting...", file=sys.stderr)
            sys.exit (1)
    debug = DebugData (debug_filename)
    data = debug.get_struct_size ('rtld_global')
    if data is None:
        print("No data found for rtld_global, exiting...", file=sys.stderr)
        sys.exit (1)
