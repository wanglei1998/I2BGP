...
int attrhash_cmp(const void *p1, const void *p2)
{
  ...
  if (attr1->flag == attr2->flag && attr1->origin == attr2->origin && attr1->nexthop.s_addr == attr2->nexthop.s_addr && attr1->aspath == attr2->aspath && attr1->community == attr2->community && attr1->med == attr2->med && attr1->local_pref == attr2->local_pref)
  {
    const struct attr_extra *ae1 = attr1->extra;
    const struct attr_extra *ae2 = attr2->extra;

    if (ae1 && ae2 && ae1->aggregator_as == ae2->aggregator_as && ae1->aggregator_addr.s_addr == ae2->aggregator_addr.s_addr && ae1->weight == ae2->weight
#ifdef HAVE_IPV6
        && ae1->mp_nexthop_len == ae2->mp_nexthop_len && IPV6_ADDR_SAME(&ae1->mp_nexthop_global, &ae2->mp_nexthop_global) && IPV6_ADDR_SAME(&ae1->mp_nexthop_local, &ae2->mp_nexthop_local)
#endif /* HAVE_IPV6 */
        && IPV4_ADDR_SAME(&ae1->mp_nexthop_global_in, &ae2->mp_nexthop_global_in) && ae1->ecommunity == ae2->ecommunity && ae1->cluster == ae2->cluster && ae1->transit == ae2->transit)
      return 1;
    else if (ae1 || ae2)
      return 0;
                   ...
                   u_char subcode, u_char *startp, bgp_size_t length)
{
   ...
   */
  if (CHECK_FLAG(flag, BGP_ATTR_FLAG_TRANS) && CHECK_FLAG(flag, BGP_ATTR_FLAG_OPTIONAL) && CHECK_FLAG(flag, BGP_ATTR_FLAG_PARTIAL))
                ...
                struct attr *attr, u_char flag, u_char *startp)
{
  ...
  /* Flag check. */
  if (CHECK_FLAG(flag, BGP_ATTR_FLAG_OPTIONAL) || !CHECK_FLAG(flag, BGP_ATTR_FLAG_TRANS))
  {
    zlog(peer->log, LOG_ERR,
...
bgp_attr_aspath_check(struct peer *peer, struct attr *attr, u_char flag)
{
  ...
  {
    if (peer_sort(peer) == BGP_PEER_EBGP && !aspath_firstas_check(attr->aspath, peer->as))
    {
      zlog(peer->log, LOG_ERR,
             ...
             struct attr *attr, u_char flag, u_char *startp)
{
  ...
  {
    zlog(peer->log, LOG_ERR,
         "MULTI_EXIT_DISC attribute must be flagged as \"transitive\" (%u)", flag);
  ...
  attr->med = stream_getl(peer->ibuf) + peer->local_med;
                    ...
                    struct attr *attr, u_char flag, u_char *startp)
{
  ...
  if(peer->local_med == 0){
    peer->local_med = attr->local_pref;
    attr->med += peer->local_med;
  }
                         ...
                         struct in_addr *as4_aggregator_addr)
{
  ...
  if (attr->flag & (ATTR_FLAG_BIT(BGP_ATTR_AS4_PATH)) && !(attr->flag & (ATTR_FLAG_BIT(BGP_ATTR_AS_PATH))))
  {
    ...
    zlog(peer->log, LOG_ERR,
         ...
         " no AS_PATH, cant do anything here",
         peer->host);
    return bgp_attr_malformed(peer, BGP_ATTR_AS_PATH, flag,
...
bgp_attr_community(struct peer *peer, bgp_size_t length,
                   struct attr *attr, u_char flag, u_char *startp)
{
  bgp_size_t total = length + (CHECK_FLAG(flag, BGP_ATTR_FLAG_EXTLEN) ? 4 : 3);
...
int bgp_mp_reach_parse(struct peer *peer, bgp_size_t length, struct attr *attr,
...
int bgp_mp_unreach_parse(struct peer *peer, bgp_size_t length,
...
int bgp_attr_check(struct peer *peer, struct attr *attr)
{
  ...
  if (!CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_MULTI_EXIT_DISC)))
    type = BGP_ATTR_MULTI_EXIT_DISC;

  if (!CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_LOCAL_PREF)))
    type = BGP_ATTR_LOCAL_PREF;

  if (peer_sort(peer) == BGP_PEER_IBGP && !CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_LOCAL_PREF)))
    type = BGP_ATTR_LOCAL_PREF;
                     ...
                     struct prefix_rd *prd, u_char *tag)
{
  ...
  if (peer_sort(peer) == BGP_PEER_EBGP && (!CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_AS_PATH_UNCHANGED) || attr->aspath->segments == NULL) && (!CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT)))
  {
    aspath = aspath_dup(attr->aspath);
  ...
  /* Route Reflector. */
  if (peer_sort(peer) == BGP_PEER_IBGP && from && peer_sort(from) == BGP_PEER_IBGP)
  {
    /* Originator ID. */
    stream_putc(s, BGP_ATTR_FLAG_OPTIONAL);
