...
bgp_pcount_adjust(struct bgp_node *rn, struct bgp_info *ri)
{
  ...
  if (rn->table->type != BGP_TABLE_MAIN || ri->peer == ri->peer->bgp->peer_self)
    return;
...
bgp_info_cmp(struct bgp *bgp, struct bgp_info *new, struct bgp_info *exist)
{
  ...
  /* 6. MED check. 在这儿把med调整到了第一个 */
  internal_as_route = (aspath_count_hops(new->attr->aspath) == 0 && aspath_count_hops(exist->attr->aspath) == 0);
  confed_as_route = (aspath_count_confeds(new->attr->aspath) > 0 && aspath_count_confeds(exist->attr->aspath) > 0 && aspath_count_hops(new->attr->aspath) == 0 && aspath_count_hops(exist->attr->aspath) == 0);

  if (bgp_flag_check(bgp, BGP_FLAG_ALWAYS_COMPARE_MED) || (bgp_flag_check(bgp, BGP_FLAG_MED_CONFED) && confed_as_route) || aspath_cmp_left(new->attr->aspath, exist->attr->aspath) || aspath_cmp_left_confed(new->attr->aspath, exist->attr->aspath) || internal_as_route)
  {
  ...
  /* 7. Peer type check. */
  if (peer_sort(new->peer) == BGP_PEER_EBGP && peer_sort(exist->peer) == BGP_PEER_IBGP)
    return 1;
  if (peer_sort(new->peer) == BGP_PEER_EBGP && peer_sort(exist->peer) == BGP_PEER_CONFED)
    return 1;
  if (peer_sort(new->peer) == BGP_PEER_IBGP && peer_sort(exist->peer) == BGP_PEER_EBGP)
    return 0;
  if (peer_sort(new->peer) == BGP_PEER_CONFED && peer_sort(exist->peer) == BGP_PEER_EBGP)
    return 0;
  ...
  if (!bgp_flag_check(bgp, BGP_FLAG_COMPARE_ROUTER_ID) && peer_sort(new->peer) == BGP_PEER_EBGP && peer_sort(exist->peer) == BGP_PEER_EBGP)
  {
...
bgp_community_filter(struct peer *peer, struct attr *attr)
{
  if (attr->community)
  {
    ...
    if (peer_sort(peer) == BGP_PEER_EBGP || peer_sort(peer) == BGP_PEER_CONFED)
                   ...
                   struct attr *attr, afi_t afi, safi_t safi)
{
  ...
  if (p->family == AF_INET && IPV4_ADDR_SAME(&peer->remote_id, &ri->attr->nexthop))
    return 0;
#ifdef HAVE_IPV6
  if (p->family == AF_INET6 && IPV6_ADDR_SAME(&peer->remote_id, &ri->attr->nexthop))
    return 0;
#endif
  ...
  if ((peer_sort(peer) == BGP_PEER_EBGP || peer_sort(peer) == BGP_PEER_IBGP || peer_sort(peer) == BGP_PEER_CONFED) && (!(attr->flag & ATTR_FLAG_BIT(BGP_ATTR_LOCAL_PREF))))
  {
  ...
  // if (peer_sort(peer) == BGP_PEER_EBGP && attr->flag & ATTR_FLAG_BIT(BGP_ATTR_MULTI_EXIT_DISC))
  // {
  ...
  /* next-hop-set */
  if (transparent || reflect || (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED) && ((p->family == AF_INET && attr->nexthop.s_addr)
#ifdef HAVE_IPV6
  ...
  else if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_SELF) || (p->family == AF_INET && attr->nexthop.s_addr == 0)
#ifdef HAVE_IPV6
           || (p->family == AF_INET6 &&
               IN6_IS_ADDR_UNSPECIFIED(&attr->extra->mp_nexthop_global))
#endif /* HAVE_IPV6 */
           || (peer_sort(peer) == BGP_PEER_EBGP && bgp_multiaccess_check_v4(attr->nexthop, peer->host) == 0))
  {
    /* Set IPv4 nexthop. */
    if (p->family == AF_INET)
    {
  ...
  if (peer_sort(peer) == BGP_PEER_EBGP && peer_af_flag_check(peer, afi, safi, PEER_FLAG_REMOVE_PRIVATE_AS) && aspath_private_as_check(attr->aspath))
                            ...
                            struct prefix *p, struct attr *attr, afi_t afi, safi_t safi)
{
  ...
  /* If this is EBGP peer and remove-private-AS is set.  */
  if (peer_sort(rsclient) == BGP_PEER_EBGP && peer_af_flag_check(rsclient, afi, safi, PEER_FLAG_REMOVE_PRIVATE_AS) && aspath_private_as_check(attr->aspath))
    attr->aspath = aspath_empty_get();
                 ...
                 afi_t afi, safi_t safi)
{
   ...
   */
  if (CHECK_FLAG(peer->bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING) && peer_sort(peer) == BGP_PEER_EBGP)
                    ...
                    int sub_type, struct prefix_rd *prd, u_char *tag)
{
  ...
  if (afi == AFI_IP && safi == SAFI_UNICAST)
  {
    ...
    if (new_attr.nexthop.s_addr == 0 || ntohl(new_attr.nexthop.s_addr) >= 0xe0000000)
    {
                ...
                struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
  ...
  /* When peer's soft reconfiguration enabled.  Record input packet in
     Adj-RIBs-In.  */
  if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_SOFT_RECONFIG) && peer != bgp->peer_self && !soft_reconfig)
  ...
  if (afi == AFI_IP && safi == SAFI_UNICAST)
  {
    /* If the peer is EBGP and nexthop is not on connected route,
	 discard it.  */
    if (peer_sort(peer) == BGP_PEER_EBGP && peer->ttl == 1 && !bgp_nexthop_check_ebgp(afi, &new_attr) && !CHECK_FLAG(peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK))
    {
    ...
    if (bgp_nexthop_self(afi, &new_attr) || new_attr.nexthop.s_addr == 0 || ntohl(new_attr.nexthop.s_addr) >= 0xe0000000)
    {
      reason = "martian next-hop;";
      goto filtered;
    }
  ...
  if (ri)
  {
    ...
    if (!CHECK_FLAG(ri->flags, BGP_INFO_REMOVED) && attrhash_cmp(ri->attr, attr_new))
    {
      ...
      if (CHECK_FLAG(bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING) && peer_sort(peer) == BGP_PEER_EBGP && CHECK_FLAG(ri->flags, BGP_INFO_HISTORY))
      {
        if (BGP_DEBUG(update, UPDATE_IN))
    ...
    if (CHECK_FLAG(bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING) && peer_sort(peer) == BGP_PEER_EBGP)
    {
    ...
    if ((afi == AFI_IP || afi == AFI_IP6) && safi == SAFI_UNICAST && (peer_sort(peer) == BGP_PEER_IBGP || peer_sort(peer) == BGP_PEER_CONFED || (peer_sort(peer) == BGP_PEER_EBGP && peer->ttl != 1) || CHECK_FLAG(peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK)))
    {
                   ...
                   struct bgp_table *table, int rsclient)
{
  ...
  if (safi != SAFI_MPLS_VPN && CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_DEFAULT_ORIGINATE))
...
int bgp_nlri_sanity_check(struct peer *peer, int afi, u_char *pnt,
                          bgp_size_t length)
{
  u_char *end;
  u_char prefixlen;
  ...
  while (pnt < end)
  {
    ...
    /* Prefix length check. */
    if ((afi == AFI_IP && prefixlen > 32) || (afi == AFI_IP6 && prefixlen > 128))
    {
      plog_err(peer->log,
...
int bgp_static_set_vpnv4(struct vty *vty, const char *ip_str, const char *rd_str,
                         const char *tag_str)
{
  int ret;
  struct prefix p;
...
/* Configure static BGP network. */
int bgp_static_unset_vpnv4(struct vty *vty, const char *ip_str,
...
void bgp_redistribute_add(struct prefix *p, struct in_addr *nexthop,
                     ...
                     struct bgp_info *binfo, afi_t afi, safi_t safi)
{
  ...
  if (attr)
  {
    ...
    if (p->family == AF_INET)
    {
      vty_out(vty, "    %s", safi == SAFI_MPLS_VPN ? inet_ntoa(attr->extra->mp_nexthop_global_in) : inet_ntoa(attr->nexthop));
    }
#ifdef HAVE_IPV6
    else
    {
                            ...
                            struct prefix_rd *prd, afi_t afi, safi_t safi)
{
  ...
  vty_out(vty, "BGP routing table entry for %s%s%s/%d%s",
          (safi == SAFI_MPLS_VPN ? prefix_rd2str(prd, buf1, RD_ADDRSTRLEN) : ""),
          safi == SAFI_MPLS_VPN ? ":" : "",
                    ...
                    const char *afi_str, const char *safi_str)
{
  ...
  if ((afi == AFI_IP && safi == BGP_SAFI_VPNV6) || (afi == AFI_IP6 && safi == BGP_SAFI_VPNV4))
  {
